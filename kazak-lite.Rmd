...
With the GFOLD score cutoff set at **`r gfold.cut`**, we got the following results for differentially expressed genes:
...
x <- vennGen("SR_up", gfold.df[gfold.up.sr, ], "SEN_up", gfold.df[gfold.up.sen, ], "Gene.Symbol.sr")

venn.diagram(x, "sr_sen_up.png", imagetype = "png", fill = c6[5:6])

```

<img src="sr_sen_up.png" width=300 height=300 />
...
#plotHeatmap(gfold.df, gfold.exprs, gfold.dn.sen.u, "SR Treated Down Only")
#dev.off()
...
g + geom_point(aes(col = GFOLD.sr + GFOLD.sen), 
    ...
    theme(panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank()) +
    labs(title = "Uniquely Up- and Down-regulated Genes in SR or SEN")
...
[Unique Up- and Down-regulated Protein-coding Genes Normalized by ACTB from GFOLD][f002]
