...
```{r plotChapter, eval =FALSE}
plotChapterHeatmap <- function(data, rowIndex, title){
    data <- data[rowIndex, ]
    mat <- as.matrix(data[, gfold.exprs])
...
```{r}
tb <- read.csv("DownPathway-NodeAttributes.csv", header = T, stringsAsFactors = F)
...
df1 = df[df$V3 > 0, ]
write.csv(df1, "DownPathway-EdgeAttributes.csv", quote = F, row.names = F)
```
