...
```{r IndividualGeneExpression, message = FALSE, fig.width = 3, fig.height = 4, warnings = FALSE}
library(bear)
#c6 <- c6.container
plotGenes <- function(x, data, dataset = "", groups){
    ...
    if(nrow(values) == 0){
        ...
        #sum.var$sd <- ifelse(is.na(sum.var$sd), 0.001, sum.var$sd)
        #sum.var$se <- ifelse(is.na(sum.var$se), 0.001, sum.var$se)
        #sum.var$ci <- ifelse(is.na(sum.var$ci), 0.001, sum.var$ci)
