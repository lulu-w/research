...
plotGenes <- function(x, data, dataset = "", groups){
    ...
    ifelse(nchar(dataset == 0), dataset, paste(" from ", dataset))
    if(nrow(values) == 0){
...
plotMore <- function(datasets, genes){
    res <- lapply(genes, plotGenes, datasets, groups = groups)
    multiplot(plotlist = res, cols = 3 )
}

lapply(datasets, plotMore, old.hk)
```

res <- lapply(ercc.genes, plotGenes, tmm.df, "TMM", groups)
res <- lapply(ercc.genes, plotGenes, deseq.df, "DESeq", groups)
res <- lapply(ercc.genes, plotGenes, loess.df, "LOESS", groups)

res <- lapply(old.hk, plotGenes, counts.df, "Raw", groups)
